...
- Every page must include `<meta name="viewport" content="width=device-width, initial-scale=1">` right after the `<meta charset="UTF-8">` line — added site-wide 2026-07-30 for mobile support. `assets/style.css` has a single shared `@media (max-width: 600px)` block (weight bars stack, TOC task-tags wrap under the title, theme toggle goes icon-only); new pages get this for free via the shared stylesheet, no per-page work needed beyond the viewport tag.
